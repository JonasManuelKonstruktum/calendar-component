...
public class DateCell extends FocusableComplexPanel
	...
	public void setHorizontalSized(boolean isHorizontalSized) {
		if (isHorizontalSized) {
			...
			width = getOffsetWidth() - WidgetUtil.measureHorizontalBorder(getElement());
	...
	private void drawDayItems(List<DateCellGroup> groups) {
		for (DateCellGroup g : groups) {
			...
			for (Integer eventIndex : g.getItems()) {
				...
				int freeSpaceCol = findFreeColumnSpaceOnLeft(new WeekGridMinuteTimeRange(
						d.getCalendarItem().getStartTime(), d.getCalendarItem().getEndTime()), order, columns);
				if (freeSpaceCol >= 0) {
					col = freeSpaceCol;
	...
	private int findFreeColumnSpaceOnLeft(WeekGridMinuteTimeRange dateRange, List<Integer> order,
		...
		for (Integer eventIndex : order) {
			...
			DateCellDayItem d = (DateCellDayItem) getWidget(eventIndex);
			WeekGridMinuteTimeRange nextRange = new WeekGridMinuteTimeRange(d.getCalendarItem().getStartTime(),
	...
	public DateCellGroup getOverlappingEvents(int targetIndex) {
		...
		WeekGridMinuteTimeRange targetRange = new WeekGridMinuteTimeRange(target.getCalendarItem().getStartTime(),
	...
	private boolean shouldDisplay(CalendarItem item) {
		...
		if (item.isTimeOnDifferentDays()) {
			...
			if (!item.getStart().equals(item.getEnd())
					&& (item.getEndTime().getHours() == 0 && item.getEndTime().getMinutes() == 0)) {
				eventEndHours = 23;
			}
	...
	public void onMouseDown(MouseDownEvent event) {
		if (event.getNativeButton() == NativeEvent.BUTTON_LEFT) {
			...
			if (e.getClassName().contains("reserved") || isDisabled()
				...
				markSlots(eventRangeStart, eventRangeStop);
				setFocus(true);
			}
		}
	}
		...
		if (dragDistance >= 0 && eventRangeStart >= 0) {
			...
			if (weekgrid.getCalendar().getRangeSelectListener() != null) {
				...
				weekSelection.setStartDay(
						DateConstants.toRPCDate(currentDate.getYear(), currentDate.getMonth(), currentDate.getDate()));
	...
	private void markSlots(int fromY, int toY) {
		Element main = getElement();
		NodeList<Node> nodes = main.getChildNodes();
		for (int i = 0; i < nodes.getLength(); i++) {
	...
	public void removeEmphasisStyle(Element elementOver) {
		String originalStylename = getStyleName(elementOver);
		setStyleName(elementOver,
				originalStylename.substring(0, originalStylename.length() - DRAGEMPHASISSTYLE.length()));
	}
	...
	public void onContextMenu(ContextMenuEvent event) {
		if (weekgrid.getCalendar().getMouseEventListener() != null) {
			...
			weekgrid.getCalendar().getMouseEventListener().contextMenu(event, DateCell.this);
		}
	}
